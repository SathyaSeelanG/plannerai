...
const RoadmapNode: React.FC<{
  ...
  const statusClasses = {
    ...
    [TaskStatus.NotStarted]: 'border-gray-600 bg-[#192f33] hover:border-primary',
  };
      ...
      <div className="flex justify-between items-start gap-2">
         <div className="flex items-start gap-3">
            ...
            <div>
                ...
                <p className="text-xs text-[#92c0c9] mt-1">{task.description}</p>
            </div>
        </div>
        <div className="flex-shrink-0 text-xs font-bold bg-[#111f22] text-[#92c0c9] px-2 py-1 rounded-full border border-gray-600">
  ...
  return (
    <>
        ...
        <div className="mb-8">
            ...
            <p className="text-[#92c0c9] mt-2 max-w-3xl">{roadmap.description}</p>
            <div className="mt-4">
            ...
            <div className="w-full bg-[#234248] rounded-full h-2.5">
        ...
        <div className="mb-8 flex justify-center">
            <div className="inline-flex rounded-md shadow-sm bg-[#111f22] p-1" role="group">
                <button
                    type="button"
                    onClick={() => setViewMode('list')}
                    className={`px-4 py-2 text-sm font-medium rounded-md transition-colors flex items-center gap-2 ${
                        viewMode === 'list' ? 'bg-primary text-[#111f22]' : 'text-white hover:bg-[#234248]'
                    }`}
                >
                ...
                <button
                    ...
                    className={`px-4 py-2 text-sm font-medium rounded-md transition-colors flex items-center gap-2 ${
                        viewMode === 'graph' ? 'bg-primary text-[#111f22]' : 'text-white hover:bg-[#234248]'
                    }`}
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {roadmap.overallResources && roadmap.overallResources.length > 0 && (
                <div className="lg:col-span-1">
                    <div className="sticky top-10 p-6 bg-[#111f22] rounded-lg border border-[#325e67]">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-xl font-bold text-white">Courses & Guides</h2>
                            <select 
                                value={sortOption} 
                                onChange={(e) => setSortOption(e.target.value as any)}
                                className="bg-[#192f33] text-white text-xs rounded border border-[#325e67] focus:border-primary outline-none px-2 py-1"
                            >
                            ...
                            {['all', 'course', 'book', 'documentation'].map(type => (
                                <button
                                    ...
                                    className={`px-2 py-1 rounded text-xs whitespace-nowrap capitalize ${
                                        filterType === type 
                                        ? 'bg-primary text-[#111f22] font-bold' 
                        ...
                        <div className="space-y-3">
                            {filteredOverallResources.length > 0 ? (
                                filteredOverallResources.map((res, i) => (
                                    <a key={i} href={res.url} target="_blank" rel="noopener noreferrer" className="flex items-start gap-4 p-3 bg-[#192f33] rounded-lg border border-transparent hover:border-primary transition-colors group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {res.rating && (
                                            <div className="flex items-center gap-1 mt-1">
                                                ...
                                                <span className="text-xs text-[#92c0c9]">{res.rating}</span>
                                            </div>
                                        )}
                                    </div>
                                    </a>
                                ...
                                <p className="text-sm text-[#92c0c9] italic">No resources found for this filter.</p>
                            )}
                        </div>
                    </div>
                </div>
